...
def get_account_access_token_async(consts, arm_access_token, permission_type='Contributor', scope='Account',
    ...
    # consts.AzureResourceManager が正しい URL であることを確認
    azure_resource_manager_url = consts.AzureResourceManager or 'https://management.azure.com'

    # URL を構築する際に必要な情報が正しくセットされているかを確認
    if not all([consts.SubscriptionId, consts.ResourceGroup, consts.AccountName, consts.ApiVersion]):
        raise ValueError("Missing necessary configuration values: SubscriptionId, ResourceGroup, AccountName, ApiVersion")  
